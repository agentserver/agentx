...
mod tests {
    ...
    // Deleted: persisted_remote_control_enrollment_round_trips_by_target_and_account
    // Deleted: clearing_persisted_remote_control_enrollment_removes_only_matching_entry
    // Reason: StateRuntime stub is a no-op (upsert/get always return Ok(())/Ok(None))
    ...
    async fn enroll_remote_control_server_parse_failure_includes_response_body() {
        ...
        let err = enroll_remote_control_server(
            ...
            &RemoteControlConnectionAuth {
                auth_provider: crate::stubs::unauthenticated_auth_provider(),
                account_id: "account_id".to_string(),
            },

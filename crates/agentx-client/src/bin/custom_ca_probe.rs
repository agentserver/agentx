...
//!
//! The shared reqwest client honors `AGENTX_CA_CERTIFICATE` and `SSL_CERT_FILE`, but those
...
//!
//! The detailed explanation of what "hermetic" means here lives in `agentx_client::custom_ca`.
//! This binary exists so the tests can exercise
//! [`agentx_client::build_reqwest_client_for_subprocess_tests`] in a separate process without
...
const PROBE_TLS13_ENV: &str = "AGENTX_CUSTOM_CA_PROBE_TLS13";
const PROBE_PROXY_ENV: &str = "AGENTX_CUSTOM_CA_PROBE_PROXY";
const PROBE_URL_ENV: &str = "AGENTX_CUSTOM_CA_PROBE_URL";
...
fn build_probe_client(
    ...
    if let Some(proxy_url) = proxy_url {
        ...
        return agentx_client::build_reqwest_client_with_custom_ca(builder.proxy(proxy))
            .map_err(|error| error.to_string());
    }

    agentx_client::build_reqwest_client_for_subprocess_tests(builder)
        .map_err(|error| error.to_string())
}

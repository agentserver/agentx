...
use agentx_api::ResponsesApiRequest;
use agentx_api::ResponsesClient;
use agentx_api::ResponsesOptions;
use agentx_client::HttpTransport;
use agentx_client::Request;
use agentx_client::RequestBody;
use agentx_client::Response;
use agentx_client::StreamResponse;
use agentx_client::TransportError;
use agentx_protocol::models::ContentItem;
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, agentx_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, agentx_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
impl AuthProvider for FailsOnceAuth {
    ...
    fn apply_auth(&self, request: Request) -> agentx_api::AuthProviderFuture<'_> {
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, agentx_client::RequestCompression::None);
    Ok(())
}
